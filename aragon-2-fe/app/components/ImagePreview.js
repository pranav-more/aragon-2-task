...
const ImagePreview = ({
  ...
  // Check if the image has multiple faces
  const hasMultipleFaces = () => {
    return (
      image &&
      image.metaData &&
      ((image.metaData.validationErrors &&
        image.metaData.validationErrors.includes("multiple_faces_detected")) ||
        (image.metaData.rejectionReason &&
          image.metaData.rejectionReason.includes("Multiple faces")))
    );
  };
  ...
  const formatImageMetadata = () => {
    ...
    if (
      ...
      const errorDescriptions = {
        ...
        blurry_image_detected: "Image too blurry",
      };
